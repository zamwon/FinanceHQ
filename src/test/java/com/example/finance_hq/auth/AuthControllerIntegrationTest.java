...
class AuthControllerIntegrationTest {
    ...
    void register_409_duplicateEmail() throws Exception {
        ...
        mvc.perform(post(AUTH_REGISTER).contentType(APPLICATION_JSON).content(body))
           .andExpect(status().isCreated());
        mvc.perform(post(AUTH_REGISTER).contentType(APPLICATION_JSON).content(body))
